...
> [!NOTE]
> Since **v0.3.0+**, Sidecar operates as a **standalone agent** with its legacy reverse-proxy engine removed.
> The proxy and tunnel mechanisms documented herein (e.g., path-prefix routing, TCP tunnel, proxy cookie-rewriting) are retained as **historical references** for older versions, or adapted to guide users deploying Sidecar behind independent reverse-proxies (like nginx, Caddy, Traefik, or Railway) or administering multiple remote Sidecar instances from a single client-side web SPA.
...
Status: updated for standalone mode in v0.3.2.
...
- API calls accidentally targeting `/api/...` instead of `/car/api/...` (frontend base-path issue)
- Cookie path scope mismatch (`Path=/api` not matching `/car/api/...`)
...
4. User switches active machine to Railway. Behaviour observed:
   ...
   a. **Thumbnail 401 at moment of switch** — file-browser still renders old GCP items briefly; their `<img src>` elements point to `https://railway.app/api/fs/thumbnail?path=<gcp-path>`. Request is cross-site subresource (`Sec-Fetch-Site: cross-site`, `Sec-Fetch-Mode: no-cors`). `SameSite=Lax` cookie is NOT sent on cross-site subresourse loads → 401.

   b. **WebSocket 401 on Terminal** — `wss://railway.app/api/ws/terminal` upgrade is cross-site. `SameSite=Lax` cookie is not sent on cross-site WebSocket upgrades → 401.

   c. **Thumbnail 401 after content loads** — Railway content fully loaded, user browses Railway folders. Thumbnail `<img>` requests to `https://railway.app/api/fs/thumbnail?path=<railway-path>` are still cross-site (page origin is still `http://34.1.134.203:3000`). Same `SameSite=Lax` cookie → NOT sent → 401.
...
**No conflict with proxied upstream subprotocols (Historical reference for pre-v0.3.0):**
The subprotocol auth applies only to sidecar's own `/api/ws/terminal` endpoint (handled by `auth_middleware` → `ws_terminal()`). In legacy versions, proxied upstream WebSocket connections went through `handle_ws_tunnel()` — a raw TCP tunnel that forwarded all headers including real `Sec-WebSocket-Protocol` values (e.g., `grpc-websockets`, `graphql-ws`) without inspection or modification.

Implemented rule (v0.3.0+):
- WS URL pattern: `${wsProtocol}//${host}${BASE_PATH}/api/ws/terminal?cols=…&rows=…`
- Frontend passes JWT as: `new WebSocket(url, ['auth-token.' + token])`
- `BASE_PATH` prefix is applied for Self-machine WS connections when behind an external reverse proxy.
...
- This applies to both the Self machine (`./api/auth/login`) and remote machines (`${machine.url}/api/auth/login`).
...
The `getReadURL(path, cacheSecs)` helper builds a URL for `GET /api/fs/read?path=<encoded>` that is directly usable as an HTML `src` attribute (e.g., `<img src>`, `<video src>`).
...
Current auth cookie path from backend auth logic: `Path=/api`

Important with path-prefix proxying (external proxy setups):
- If browser route is `/car/api/...`, cookie with `Path=/api` does not match
- The external proxy must rewrite upstream `Set-Cookie Path=` when request is under prefix routes
- v0.1.5 behavior (Historical): prepend route prefix
    - example: `Path=/api` -> `Path=/car/api`
...
1. Frontend calls `POST /api/auth/logout` (or `./api/auth/logout` for Self) with `credentials: 'include'`.
...
`BASE_PATH` is computed once at module load from `window.location.pathname` by extracting the trailing filename (like `index.html` or `app.js`) and trailing slashes. This allows Sidecar to be correctly served behind any arbitrary path prefix:
...
Example outcomes:
- Served at `/` -> `BASE_PATH = ""`
- Served behind proxy prefix `/car/` -> `BASE_PATH = "/car"`
- Served behind proxy prefix `/x/y/` -> `BASE_PATH = "/x/y"`
...
Without this frontend fix, API calls would target `/api/…` instead of `/car/api/…`, causing 404s or misroutes even after the backend cookie path is correctly rewritten.
...
### Partition Isolation Nullifies Cross-Mode Cache Reuse (2026-07 Analysis)

**Claim**: "Host-as-relay proxy mode causes browser-side cache loss compared to direct cross-origin access mode."

**Conclusion**: This claim is **false**. In Sidecar's current auth architecture, cross-mode cache reuse is *already impossible* in both proxy mode and direct-access mode due to layered partition isolation. The proxy mode introduces **no additional cache penalty**.

#### Scenario

1. User opens Host Machine A's SPA, connects to Remote Machine B through the multi-machine panel.
2. One hour later, user navigates directly to Machine B's own SPA (`https://machine-B/admin`).

Question: Can the browser reuse anything from step 1 in step 2?

#### Analysis — Four Layers of Isolation

**Layer 1: JWT (localStorage — per-origin)**

JWT is stored via `localStorage.setItem('auth_token_<machineName>', token)` in `storage.js`. `localStorage` is scoped per-origin by the Web Storage API spec. The key `auth_token_B` stored under `https://host-A` is invisible when the browser loads `https://machine-B`.

→ **Direct access to B finds no JWT.** User must re-authenticate. This holds regardless of proxy or direct mode.

**Layer 2: Cookie (`Partitioned` attribute — CHIPS)**

Under HTTPS, `build_set_cookie()` in `auth.rs` sets `Secure; SameSite=None; Partitioned`. The `Partitioned` attribute (Cookies Having Independent Partitioned State, CHIPS) creates cookie storage keyed by `(top-level-site, cookie-domain)`:

| Access mode | top-level-site | cookie-domain | Partition key |
|-------------|---------------|--------------|---------------|
| Via Host A (indirect) | `host-A.example.com` | `machine-B.example.com` | `(A, B)` |
| Direct to B | `machine-B.example.com` | `machine-B.example.com` | `(B, B)` |

Different partition keys → cookie from step 1 is invisible in step 2.

→ **Even without `Partitioned`, `SameSite=None` third-party cookies are dropped by iOS Safari ITP.** The isolation holds with or without the `Partitioned` attribute.

**Layer 3: Fetch Token (HMAC — double-locked)**

The fetch token (used for WebKit media URL auth on `/fs/read` and `/fs/thumbnail`) is stored in `localStorage` (`fetch_token_<machineName>`) — same per-origin isolation as JWT.

Additionally, the fetch token is HMAC-bound to the `Referer` header at generation time (`generate_fetch_token(key, referer)` in `auth.rs`) and validated against `Referer` on every request:

- Via Host A: `Referer: https://host-A.example.com/admin/...`
- Direct to B: `Referer: https://machine-B.example.com/admin/...`

→ **Different Referer → HMAC validation fails.** Even if the token were somehow transported, it cannot be reused.

**Layer 4: HTTP Cache (Network Isolation Key)**

Modern browsers (Chrome 86+, Firefox 85+) partition the HTTP cache using the **Network Isolation Key**: `(top-level-site, current-frame-site, URL)`:

| Access mode | top-level-site | frame-site | Cache key |
|-------------|---------------|-----------|-----------|
| Via Host A → B thumbnail | `host-A` | `machine-B` | `(A, B, url)` |
| Direct to B thumbnail | `machine-B` | `machine-B` | `(B, B, url)` |

→ **Same URL, different top-level-site → cache miss.** The browser fetches the thumbnail from the network as if it has never been seen before.

#### Summary

| Layer | Reusable across modes? | Mechanism |
|-------|----------------------|-----------|
| JWT (localStorage) | ❌ No | Per-origin Web Storage |
| Cookie | ❌ No | CHIPS partitioning + ITP |
| Fetch Token | ❌ No | Per-origin storage + HMAC-Referer binding |
| HTTP Cache | ❌ No | Network Isolation Key partitioning |

**All four layers are isolated. Direct access to a previously-indirectly-accessed machine is always a fresh session with zero cache reuse.**

#### Implication for Proxy Mode vs Direct Mode

The "browser cache advantage" of direct cross-origin access was never real. Therefore, when evaluating whether to adopt Host-as-relay proxy mode (all traffic routed through Host, same-origin for the browser), the cache-loss argument carries **zero weight**. The only real cost of proxy mode is the network topology cost (additional hop, increased latency, doubled bandwidth for file transfers) — not a cache cost.
...
- "Does Sec-WebSocket-Protocol auth conflict with proxied upstream real subprotocols?" -> No; subprotocol auth is scoped to sidecar's own `/api/ws/terminal` handler; proxied upstream WS (in legacy proxy) went through raw TCP tunnel (`handle_ws_tunnel`) which forwarded all headers including real subprotocols without inspection
...
- "Does proxy mode cause browser cache loss vs direct cross-origin access?" -> No; cross-mode cache reuse is already impossible due to four-layer partition isolation (localStorage per-origin, CHIPS cookie partitioning, HMAC-Referer fetch token binding, Network Isolation Key cache partitioning); see "Partition Isolation Nullifies Cross-Mode Cache Reuse" analysis above
...
- Proxy cookie path adaptation: enabled for prefixed proxy routes (Historical)
- URL query auth: permanently disabled (HTTP API and WebSocket)
- WebSocket auth: `Sec-WebSocket-Protocol: auth-token.<jwt>` subprotocol header (primary, v0.1.6c) with cookie fallback; no `?token=` in URL. Fixes iOS Safari ITP intermittent cookie suppression. Subprotocol auth scoped to `/api/ws/terminal` only — does not conflict with proxied upstream real subprotocols.
...
- WebSocket proxy implementation: raw TCP tunnel (`hyper::upgrade::on()` + `tokio::io::copy_bidirectional`, v0.1.6b); auth headers (`Cookie`, `Authorization`) forwarded verbatim in upstream upgrade request; hop-by-hop WS headers regenerated fresh; `tokio-rustls` for wss:// upstreams; no WS frame parsing in proxy (Historical reference for pre-v0.3.0)
