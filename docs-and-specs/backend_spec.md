# SidecarX - Backend Specification & API Documentation

**Version:** 0.1.0

## Overview

SidecarX is a lightweight, high-performance Rust server designed to be deployed as a standalone agent alongside your applications (e.g., containerized tools, chatbots, or admin interfaces). It provides secure, stateless, authenticated file management, remote terminal access, and system monitoring in a single static binary.

SidecarX operates as a standalone agent. It serves its built-in admin Single Page Application (SPA) natively, sandboxes operations using optional root-jailing, and provides real-time system stats.
...
2. **`/admin` / `/admin/`** — Permanent redirect to the root `/` (for backward-compatible pathing).
...
SidecarX can be configured through local environment variables or a `.env` file placed in the working directory:
...
| `SIDECAR_SECRET` | — | Master password used for login and JWT signing. (**Required in production!**) |
| `SIDECAR_PANEL_DIR` | `./admin` | Path to the directory containing static admin panel SPA assets. |
| `SIDECAR_ROOT_DIR` | — | Jail directory. If set, restricts all filesystem operations strictly within this path. |
...
SidecarX enforces a robust multi-layered authentication schema for all API endpoints (excluding `/api/auth/login` and `/api/auth/logout`).

### 1. Same-OS Loopback Auth Bypass
...
5. If none is valid → **`410 Unauthorized`** (or relevant auth error code)
...
- **Behavior:** Accepts raw binary data in the request body and streams it directly to disk (using a bounded memory footprint). Automatically creates parent directories if missing.
...
- **Behavior:** Recursively traverses directories starting at `path` using `walkdir`, matching names against the glob `pattern`.
- **Response Body:**
  ```json
  {
    "matches": [
...
- **Cache Mechanics:** Validated under Freedesktop thumbnail standards (`~/.cache/thumbnails/x-large/`). Small images (<100 KB) bypass generation and are returned directly to optimize performance.
- **Security Guard:** Enforces thumbnail-loop protection to prevent generating thumbnails of thumbnails recursively. Includes standard URL percent-encoding on embedded tEXt metadata to prevent panics when handling non-ASCII/Unicode filepaths.
...
- **Behavior:** Compresses and streams files directly as a ZIP archive. Memory footprint is strictly $O(1)$ and does not scale with file or directory size.
...
- **Response Body:**
  ...
  ```
- **Metrics Description:**
  - `cpu_usage`: Percentage representation of overall CPU utilization.
